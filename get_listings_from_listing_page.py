...
"""Get all individual listings from every listings page."""
...
def get_listings_on_page(listings_dir, listing_page, domain):
    """
    Fetch all individual listings on a listing page,
    ...
    for listing_table_row in listing_page.find_all(class_='casa_repeater'):
        ...
        listing_address = listing_table_row.find(class_='renta_descripcion')\
                                           .a['href']
...
def main(listing_pages_dir, listings_dir, listing_page_files):
    ...
    # Loop through listing page.
    domain = 'http://www.cubisima.com'
    for listing_page_file in listing_page_files:

        # Parse HTML of given listings page.
        listing_page = BeautifulSoup(open(listing_pages_dir + '/' +
                                          listing_page_file, 'r'))
        # Get listings on that page, save files to disk.
        get_listings_on_page(listings_dir, listing_page, domain)


if __name__ == '__main__':
    # All listing pages passed in through xargs.
